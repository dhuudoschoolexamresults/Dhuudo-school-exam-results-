...
function positionOf(result, data) {
  const sameExam = data
    .filter(x =>
      String(x["Exam"] ?? "") === String(result["Exam"] ?? "")
    )
    .map(x => ({
  ...
  const position = sameExam.findIndex(
    x =>
      x.id ===
      String(result["ID"] ?? "").trim().toLowerCase()
  );
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      ...
      const calculated = totalAndAverage(result);
      ...
      const average =
        Number(result["Average"] ?? calculated.average);
      ...
      document.getElementById("studentInfo").innerHTML = `
        ...
        <strong>Roll Number:</strong> ${result["ID"] ?? ""}<br>
